...
<Note>
  Replace `<version>` with the specific release tag you want to use, for example `0.1.0`. Pinning the version in CI ensures reproducible builds.
</Note>
...
<Note>
  Shell completions for Bash, Zsh, Fish, Elvish, and Nushell can be enabled with `acton completions <shell>`. See `acton completions --help` for details.
</Note>
